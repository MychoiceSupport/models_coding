...
def generate_raw_data():
    with open('../data/ottWithSignaling_0306_1000.json','r') as f:
    ...
    X = np.array(X)
    Y = np.array(Y)
    np.save('../data/all_data_X_origin.npy',X)
    np.save('../data/all_data_y_origin.npy',Y)
...
def generate_training_info(X, y, dimention):
    ...
    laccell = X[:,:,2]
    shape = laccell.shape
    laccell = laccell.reshape(-1,).tolist()
    number = list(set(laccell))
    hash_dict = dict(zip(number, range(len(number))))
    print(len(hash_dict))
    laccell_ids = np.array([hash_dict[i] for i in laccell]).reshape(shape)
    # print(time_start_end)
    # get_shape = time_start_end.shape
    # time_start_end_0 = pd.Series(pd.to_datetime(time_start_end[:,:,0].reshape(-1,)))
    # time_start_end_0 = time_start_end_0.apply(lambda x: (x - pd.Timestamp("1970-01-01")) // pd.Timedelta('1s')).to_numpy()
    # time_start_end_1 = pd.Series(pd.to_datetime(time_start_end[:,:,1].reshape(-1,)))
    # time_start_end_1 = time_start_end_1.apply(lambda x: (x - pd.Timestamp("1970-01-01")) // pd.Timedelta('1s')).to_numpy()
    ...
    print("查看laccell_ids的形状",laccell_ids.shape)
    np.save('../data/lac_id.npy', laccell_ids)
...
class traj_data(Dataset):
    def __init__(self,  X, y, time_delta, time_feature, laccell_ids,sample=False, data_length=11, device = 'cpu',target_dim=2, mode = 'train', load_from_exist = False):
        ...
        if mode == 'train':
            if sample:
                train_index = np.load('../data/train_index_sample.npy',allow_pickle=True)
            else:
                train_index = np.load('../data/train_index.npy',allow_pickle=True)
            ...
            current_laccell_ids = laccell_ids[train_index]
        elif mode == 'val':
            if sample:
                val_index = np.load('../data/val_index_sample.npy',allow_pickle=True)
            else:
            ...
            current_laccell_ids = laccell_ids[val_index]
        elif mode == 'test':
            if sample:
                test_index = np.load('../data/test_index_sample.npy',allow_pickle=True)
            else:
            ...
            current_laccell_ids = laccell_ids[test_index]
        ...
        self.lac_id = current_laccell_ids.astype(np.int64)
    ...
    def __getitem__(self, index):
        ...
        lac_id = self.lac_id[index]
        # extra_features = self.extra_features[index]
        sequence_data = (
            observed_data, ground_truth, time_delta, time_start_end, lac_id
        )
...
def get_the_dataloader(X,y,time_delta,time_feature,lac_id,sample,batch_size,target_dim = 2, device='cpu', num_workers = 0):
    train_dataset = traj_data(X,y,time_delta,time_feature,lac_id, sample, mode='train',target_dim=target_dim, device=device)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, num_workers=num_workers, shuffle=True, pin_memory=True)
    val_dataset = traj_data(X,y,time_delta,time_feature,lac_id,sample,mode='val', target_dim=target_dim, device=device)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers, shuffle=True, pin_memory=True)
    test_dataset = traj_data(X,y,time_delta,time_feature,lac_id,sample, mode='test', target_dim=target_dim, device=device)
...
if __name__ == '__main__':
    ...
    # # # print(val_index)
    # # print("进入baocun")
    # np.save('../data/train_index_sample.npy', train_index)
    # np.save('../data/test_index_sample.npy', test_index)
    # np.save('../data/val_index_sample.npy', val_index)
    # generate_scaler()
